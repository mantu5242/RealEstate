...
const Home = () => {
  ...
  useEffect(() => {
    const fetchOfferListings = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/listing/get?offer=true&limit=4');
    ...
    const fetchSaleListings = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/listing/get?type=sale&limit=4');
        const data = await res.json();
